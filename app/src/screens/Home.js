...
export default function Home({ navigation }) {
  const { F, sym, profile, pots, expenses, totalSpend, monthBudget, repos } = useApp();
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setNextTrip(await repos.trips.next());
        setStreak(await repos.expenses.streakDays());
        setTopMover(await repos.items.topMover());
      } catch {}
    })();
  }, [expenses, repos]);
