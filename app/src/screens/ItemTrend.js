...
export default function ItemTrend({ route, navigation }) {
  ...
  useEffect(() => {
    (async () => {
      const h = await repos.items.priceHistory(normalizedName);
      setHistory(h);
      const s = await repos.items.stats(normalizedName);
      setStats(s);
    })();
  }, [normalizedName, repos]);

  useEffect(() => {
    repos.items.consumption(normalizedName, { bucket, range: 12 }).then(setConsumption);
  }, [normalizedName, bucket, repos]);
