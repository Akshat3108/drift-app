...
export const accounts = {
  async list() {
    return all(`SELECT * FROM accounts WHERE ${NOT_DELETED} ORDER BY kind, id`);
  },
  async get(id) {
  ...
  async remove(id) {
    await exec(`UPDATE accounts SET deleted_at = datetime('now') WHERE id = ? AND deleted_at IS NULL`, [id]);
  },
  async restore(id) {
    await exec('UPDATE accounts SET deleted_at = NULL WHERE id = ?', [id]);
  },
  async netWorth() {
    const row = await one(`
      SELECT
