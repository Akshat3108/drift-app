...
function EditTrip({ route, navigation }) {
  const { F, sym, trips, addTrip, updateTrip, removeTrip, restoreTrip, settings } = useApp();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast(`Deleted: ${label}`, {
        actionLabel: 'Undo',
        onAction: async () => {
          try { await restoreTrip(id); }
          catch (err) {
            logError('edittrip:undo-delete', err);
            Alert.alert('Restore failed', err?.message || String(err));
          }
        },
      });
    } catch (err) {
      logError('edittrip:delete', err);
...
export default React.memo(EditTrip);
