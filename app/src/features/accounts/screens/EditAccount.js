...
function EditAccount({ route, navigation }) {
  const { F, sym, accounts, addAccount, updateAccount, removeAccount, restoreAccount } = useApp();
  ...
  const handleDelete = async () => {
    const labelText = label.trim() || 'account';
    const id = editing.id;
    try {
      await removeAccount(id);
      navigation.goBack();
      toast(`Deleted: ${labelText}`, {
        actionLabel: 'Undo',
        onAction: async () => {
          try { await restoreAccount(id); }
          catch (err) {
            logError('editaccount:undo-delete', err);
            Alert.alert('Restore failed', err?.message || String(err));
          }
        },
      });
    } catch (err) {
      logError('editaccount:delete', err);
